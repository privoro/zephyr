...
static void channel_irq(edma_handle_t *handle)
{
	...
	if (handle->tcdPool == NULL) {
		...
		u32_t sga = handle->base->TCD[handle->channel].DLAST_SGA;
		u32_t sga_index;
		s32_t tcds_done;
		u8_t new_header;

		sga -= (u32_t)handle->tcdPool;
		sga_index = sga / sizeof(edma_tcd_t);
		/* Adjust header positions. */
		if (transfer_done) {
			new_header = (u8_t)sga_index;
		} else {
			new_header = sga_index != 0U ?
					     (u8_t)sga_index - 1U :
					     (u8_t)handle->tcdSize - 1U;
		}
		/* Calculate the number of finished TCDs */
		if (new_header == (u8_t)handle->header) {
			s8_t tmpTcdUsed = handle->tcdUsed;
			s8_t tmpTcdSize = handle->tcdSize;
			...
			tcds_done = (u32_t)new_header - (u32_t)handle->header;
			if (tcds_done < 0) {
...
/* Configure a channel */
static int dma_mcux_edma_configure(struct device *dev, u32_t channel,
...
static int dma_mcux_edma_start(struct device *dev, u32_t channel)
{
	struct call_back *data = DEV_CHANNEL_DATA(dev, channel);
...
static int dma_mcux_edma_stop(struct device *dev, u32_t channel)
{
...
static int dma_mcux_edma_reload(struct device *dev, u32_t channel, u32_t src,
